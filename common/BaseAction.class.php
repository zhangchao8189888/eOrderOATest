...
class BaseAction
{
    ...
    {
        global $productType;
        $this->objForm = new BaseForm();
        $this->objForm->setFormData("productTypeList",$productType);
    }
    function initBase()
    {
    	//开始SESSION
        ...
        if (!empty($user)) {
            ...
            $groupId = $groupDao->getUserGroup($user['id']);
            $rules = $groupDao->getUserGroupAccessPO($groupId);
            $access = json_decode($rules['access_json'],true);
            if (!empty($_REQUEST["n"])) {
                $pages = explode("_",$_REQUEST["n"]);
                $this->access_name = $access[$pages[0]][$pages[1]];
            }
            if(!empty($rules)){
                $user_auth_list = explode(',',$rules['rules']);
            }
            ...
            if (in_array($user['id'],$checkPhoneNum)) {
                $this->checkPhoneNum = true;
            }

        }
    }
    private function initVal() {
